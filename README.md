...
Логи пишутся в **stdout** (консоль) по умолчанию. Уровень логирования настраивается в `main.py` и `llm/client.py`.

> ⚠️ **Известная проблема:** `logging.basicConfig()` вызывается в двух файлах (`main.py` и `llm/client.py`). Второй вызов (`DEBUG` в `client.py`) перезаписывает настройки первого (`INFO` в `main.py`). В результате логи всегда пишутся на уровне DEBUG. Для исправения уберите `logging.basicConfig` из одного из файлов.
...
| `tests/test_config.py` | Проверка конфигурации и зависимостей | 5 |
...
- Существование необходимых файлов проекта
